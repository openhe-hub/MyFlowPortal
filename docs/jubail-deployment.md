...
Generated by `scripts/generate_h2s_manifest.py` with seed=42:
...
- **Random prompt** from 10 target backgrounds (see `scripts/h2s_prompts.txt`)
...
tar cf - -T /home/nyuair/zhewen/FlowPortalRelease/scripts/h2s_selected_videos.txt | \
  ssh chatsign@10.224.35.17 "ssh jubail 'cd /scratch/zl6890/Workspace/zhewen/FlowPortal/datasets/How2Sign/train_mp4 && tar xf -'"
```

3. Transfer scripts:
```bash
for f in scripts/h2s_manifest.csv scripts/h2s_manifest_chunk{0,1,2,3}.csv scripts/h2s_prompts.txt run-h2s-batch-slurm.sh; do
